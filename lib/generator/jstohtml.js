...
const jsToHtmlWith = (sourcePath, elmcode, appjsPath, withDraft, autoReloader) => {
    try {
        ...
        return html_minifier_1.minify(html, { minifyCSS: true, minifyJS: true });
    }
    catch (e) {
        console.error('error:');
 ...
 * @param dom
 */
const unescapeScripts = (dom) => {
    //return  html.replace(/siteelm-custom-/g, '')
    const customs = dom.window.document.querySelectorAll('siteelm-custom[data-tag="script"]') || [];
    customs.forEach((x) => {
